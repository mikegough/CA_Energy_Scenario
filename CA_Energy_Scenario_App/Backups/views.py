...
def index(request):
    ...
    ############################################ BUILD SQL EXPRESSION ##################################################
        ...
        ####################### Experiment 1km grid cell query ########################################################
        ...
        WKT_SelectedPolys=WKT_SelectedPolys.replace('%', ' ')
        WKT_SelectedPolys="SRID=4326;"+WKT_SelectedPolys

        cursor = connection.cursor()

        query1="create temp table temp1 as SELECT ST_Union(geom) as the_geom from drecp_reporting_units_1km_poly_v2 where intactness <=" + ti_slider + " and ST_Intersects('"+ WKT_SelectedPolys + "', " + "drecp_reporting_units_1km_poly_v2" + ".geom)"
        ...
        query3="alter table temp2 add area float; " \
               "update temp2 set area = st_area(geom2) * 111000 * 111000 * cos(37.229722) * power((39.37/12 / 5280),2) * 640 ; " \
               "delete from temp2 where area < " + str(min_area) + ";"
