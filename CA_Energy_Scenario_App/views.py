...
def get_reporting_units_param(reporting_units):
    ...
    return reporting_units_table, name_field


def query(variables):
    WKT = variables.get('wktPOST')
    reporting_units = variables.get('reporting_units', 'onekm')
    reporting_units_table, name_field = get_reporting_units_param(reporting_units)

    cursor = connection.cursor()
    select_list = "SELECT "

    #TODO remove stats_fields (it's not used)
    stats_fields = variables['stats_fields']
    if stats_fields:
    ...
    if WKT == 'aspatial':
        ...
        query_field = variables.get('queryField')
        operator = variables.get('operator').strip()
        string_or_value = variables.get('stringOrValue').lower().strip()
    ...
    cursor = connection.cursor()
    query_layer = variables['query_layer']
    solar_slider_value = variables['solar_slider_value']
    ownership_values = variables.get('ownership_values', 'BLM,Military,Native American,Other,Private,State Land,USFS')
    enable_environment_settings = bool(variables['enable_environment_settings'])
    ti_slider = variables['ti_slider']
    cv_slider = variables['cv_slider']
    species_count_slider_value = variables['species_count_slider_value']
    chat_slider_value = variables['chat_slider_value']
    distance_to_transmission = variables['distance_to_transmission']
    min_area = float(variables['min_area'])
    min_area_units = variables.get('min_area_units')
    ...
    distance_to_transmission_units = variables.get('distance_to_transmission_units')
    ...
    query3 = """
        ALTER TABLE temp2 ADD area FLOAT;
        UPDATE temp2 SET area = st_area(geom2::GEOGRAPHY);
        DELETE FROM temp2 WHERE area < {0};
     """.format(min_area * area_conversion_factor)
    ...
    return reporting_units, WKT_search_area, WKT_selected_polys, area_acres, count, results_json, features_names
...
def index(request):
    ...
    query_kwargs['query_layer'] = "energy_scenario_1km_query_grid"

    query_kwargs['stats_fields'] = None
    ...
    ############################################ BUILD SQL EXPRESSION ##################################################
        ...
        reporting_units, WKT_search_area, WKT_selected_polys, area_acres, count, results_json, features_names = query(query_kwargs)
